...
async function databaseArchiveVotes(rows = []) {
  if (!rows || !rows.length) return;
  const client = getSupabaseClient();
  const now = new Date().toISOString();
  if (client) {
    const payload = rows.map((r) => ({
      original_id: r.id || null,
      user_id: r.user_id || null,
      category: r.category || null,
      value: r.value || null,
      date: r.date || null,
      comment: sanitizeComment(r.comment) || null,
      created_at: r.created_at || null,
      archived_at: now,
    }));
    const { error } = await client.from("archived_votes").insert(payload);
    if (error) throw new Error(error.message || "Erreur insertion archived_votes");
    return;
  }

  const pool = getPgPool();
  if (!pool) throw new Error("Aucune base de données configurée");
  const insertQuery = `INSERT INTO archived_votes
    (original_id, user_id, category, value, date, comment, created_at, archived_at)
  ...
  for (const r of rows) {
    await pool.query(insertQuery, [
      ...
      r.value || null,
      r.date || null,
      sanitizeComment(r.comment) || null,
      r.created_at || null,
      now,
    ]);
  }
...
export async function resetUserVotesForDate(userId, date) {
  ...
  try {
    if (rows.length) {
      if (backend === "database") {
        await databaseArchiveVotes(rows);
      }
      // always archive to fallback as well for redundancy
      await fallbackArchiveVotes(rows);
    }
  } catch (err) {
...
export async function readArchived() {
  ...
  if (backend === "database") {
    ...
    if (client) {
      const { data, error } = await client
        .from("archived_votes")
        .select("original_id, user_id, category, value, date, comment, created_at, archived_at")
        .order("archived_at", { ascending: false })
        .limit(1000);
      if (!error) {
        return (data || []).map((r) => ({ archived_at: r.archived_at, original: {
          id: r.original_id,
          user_id: r.user_id,
          category: r.category,
          value: r.value,
    ...
    if (pool) {
      const { rows } = await pool.query(
        "SELECT original_id, user_id, category, value, date, comment, created_at, archived_at FROM archived_votes ORDER BY archived_at DESC LIMIT 1000"
      );
      return (rows || []).map((r) => ({ archived_at: r.archived_at, original: {
        id: r.original_id,
        user_id: r.user_id,
        category: r.category,
        value: r.value,
